//Go has built-in support for JSON encoding and decoding, including to and from built-in and custom data types.
...
//Use these two structs to demonstrate encoding and decoding of custom types below.
...
func main() {
	...
	//Look at encoding basic data types to JSON strings. Here are some examples for atomic values.
	...
	//Use tags on struct field declarations to customize the encoded JSON key names. Check the definition of response2 above to see an example of such tags.
	res2D := &response2{
		Page:   1,
		Fruits: []string{"apple", "peach", "pear"}}
	res2B, _ := json.Marshal(res2D)
	fmt.Println(string(res2B))
	//Look at decoding JSON data into Go values. Here’s an example for a generic data structure.
	byt := []byte(`{"num":6.13,"strs":["a","b"]}`)
	//Provide a variable where the JSON package can put the decoded data. This map[string]interface{} will hold a map of strings to arbitrary data types.
	...
	fmt.Println(dat)
	//To use the values in the decoded map, convert them to their appropriate type. For example here we convert the value in num to the expected float64 type.
	num := dat["num"].(float64)
	fmt.Println(num)
