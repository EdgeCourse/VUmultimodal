//https://go.dev/play/p/n_Rsx1Rvh-E
/*
Drop Pattern
...
A good use case for this pattern would be a DNS server. A DNS server has a limited capacity, or limited amount of requests that it can process at any given moment. If there are more requests sent to the DNS server we can decide to overload and kill the server, or to DROP new requests until DNS server has capacity to process the request.
